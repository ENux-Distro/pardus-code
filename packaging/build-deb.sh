...
#
# Requires the PARDUS-branded opencode binary at vendor/opencode. Build it once
# on a capable machine with:  packaging/build-opencode.sh
# The resulting .deb bundles that binary, so end users install with zero
# compiling (works fine on low-RAM machines).
#
set -euo pipefail
...
ARCH="amd64"
PKG="pardus-code"
MAINTAINER="Emir YILDIRIM <emir@emirpasha.com>"
...
# --- shared assets ---------------------------------------------------------
for f in pardus.tmux.conf pardus.nanorc lang.sh pardus.tui.json opencode.jsonc \
         WELCOME.md WELCOME.tr.md; do
